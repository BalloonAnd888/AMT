...
def rgb_arr_to_rgb_pixmap(arr):
    ...
    img = QtGui.QImage(arr.data, w, h,
                       arr.strides[0], QtGui.QImage.Format.Format_RGB888)
...
class SpectrogramView(QtWidgets.QGraphicsView):
    ...
    ASPECT_RATIO = QtCore.Qt.AspectRatioMode.IgnoreAspectRatio
...
class RollView(MouseEventManager, SpectrogramView):
    ...
    VGRID_ZVAL = 1
    #
    SELRECT_PEN = QtCore.Qt.PenStyle.NoPen  # QtCore.Qt.GlobalColor.yellow
    VGRID_CENTER_STYLE = QtCore.Qt.PenStyle.SolidLine
    VGRID_SIDE_STYLE = QtCore.Qt.PenStyle.DashLine
    ...
    def create_vline(self, pos_x, width=0, zval=1,
                     style=QtCore.Qt.PenStyle.SolidLine, rgba=(255, 0, 0, 100)):
        """
        """
...
class MultiSpectrogramControls(QtWidgets.QWidget):
    ...
    def set_cbox_cmap(cbox, cmap_name):
        ...
        cmap_idx = cbox.findText(
            cmap_name, QtCore.Qt.MatchFlag.MatchFixedString)
        if cmap_idx >= 0:
